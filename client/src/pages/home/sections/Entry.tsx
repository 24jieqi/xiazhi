...
export default () => {
  const [pageAllPublicEntries, { refetch }] = usePageAllPublicEntriesLazyQuery()
  const actionRef = useRef<ActionType>()
  const [editableKeys, setEditableRowKeys] = useState<React.Key[]>([])
  const columns: ProColumns<EntryItem>[] = useMemo(() => {
    return [
      {
        title: '词条Key',
        dataIndex: 'key',
  ...
  return (
    <ProTable<EntryItem>
      ...
      headerTitle="公共词条库"
      toolBarRender={() => [
        <EntryModal key="new" onActionSuccess={refetch}>
